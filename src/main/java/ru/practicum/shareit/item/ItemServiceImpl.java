...
public class ItemServiceImpl implements ItemService {
    ...
    public List<ItemDto> getOwnersItems(Integer userId) {
        UserValidate.checkUserId(userRepository.getUsersMap().keySet(), userId);
    ...
    public List<ItemDto> searchBy(String text) {
        ...
        return itemRepository.searchBy(text).stream()
                .map(ItemMapper::toItemDto)
                .collect(Collectors.toList()
                );
    }
    ...
    public ItemDto getItemById(Integer id) {
        ItemValidate.checkItemId(itemRepository.getItemsMap().keySet(), id);
    ...
    public ItemDto updateItem(Integer itemId, Integer userId, ItemDto itemDto) {
        UserValidate.checkUserId(userRepository.getUsersMap().keySet(), userId);
        ItemValidate.checkItemId(itemRepository.getItemsMap().keySet(), itemId);
        Item item = itemRepository.getItemsMap().get(itemId);
        ItemValidate.checkOwnerItem(item, userId);
        Item updater = itemRepository.updateItem(itemId, ItemMapper.dtoToItem(itemDto, item));
